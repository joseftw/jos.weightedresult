...
It's **highly recommended** that you only create one ```WeightedResultQuery<T>``` instance per type, preferably you register it as a **singleton** in your favorite DI contanier or use it as a static field. We want to avoid creating ```AliasMethodVose``` instances frequently since it does some initialization in the constructor.
